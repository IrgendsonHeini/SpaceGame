...
class GameLoop:
    ...
    def redraw_window(self):      
        ...
        player.move_lasers(-self.laser_vel*2, gl.fleet)
    ...
    def spawn_tiefighter(self):
        for i in range(self.fleet_length):
            x = random.randrange(50,WIDTH-SMALL-50,SMALL)
            y = random.randrange(-400-SMALL*self.level,-100,SMALL)                               
    ...
    def spawn_tiehunter(self):
        if self.level >= 7:
            self.wave_length = int(self.fleet_length/4)
            for i in range(self.wave_length):
                ...
                del gl.fleet[0:self.wave_length]

    def spawn_imperialshuttle(self):
        if self.level >= 4:
            self.wave_length = int(self.fleet_length/5)
            for i in range(self.wave_length):
                x = random.randrange(50,WIDTH-100-50,100)
                y = random.randrange(-300-50*self.level,-100,200)  
    ...
    def spawn_stardestroyer(self):
        if self.level >= 11:
            self.wave_length = int(self.fleet_length/6)
            for i in range(self.wave_length):
                x = random.randrange(50,WIDTH-SMALL-50,MEDIUM)
                y = random.randrange(-300-SMALL*self.level,-100,200)                      
    ...
    def spawn_slaveone(self):
        if self.level == 20:
            for i in range(int(1)):
                x = random.randrange(50,WIDTH-100-50,100)
                y = random.randrange(-700-100*self.level,-100,100)                      
...
# Hauptschleife
    ...
    # Game over       
            ...
            if self.lost:
                ...
                if self.lost_count <= 9:    
                    player.ship_img = EXPLOSIONS[int(self.lost_count)]
                
                self.lost_count += 0.3 
                if self.lost_count > 9:
                    player.ship_img = EXPLOSIONS[9]
                               
                if self.lost_count > FPS:
                    self.active = False
                    score_window()
    ...
    # Gegner einfügen
            ...
            if len(gl.fleet) == 0:
                ...
                self.spawn_tiefighter()
                self.spawn_imperialshuttle()
                self.spawn_tiehunter()            
                self.spawn_stardestroyer()
                self.spawn_deathstar()
                self.spawn_slaveone()
    ...
    # Gegner           
            ...
            for enemy in gl.fleet[:]:
                ...
                enemy.move_lasers(self.laser_vel, player)
