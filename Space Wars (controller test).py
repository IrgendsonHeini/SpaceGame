...
# Steuerung
        ...
        if not keys[pygame.K_RIGHT] or joystick.get_axis(0) > 0.5 and not keys[pygame.K_LEFT] or joystick.get_axis(0) < -0.5:
            player.ship_img = MILLENNIUM_FALCON
            
        if keys[pygame.K_LEFT] or joystick.get_axis(0) < -0.5 and keys[pygame.K_RIGHT] or joystick.get_axis(0) > 0.5:
            player.ship_img = MILLENNIUM_FALCON
                       
        if keys[pygame.K_UP] or joystick.get_axis(1) < -0.5:
            player.y -= player_vel
            
        if keys[pygame.K_DOWN] or joystick.get_axis(1) > 0.5:
            player.y +=  player_vel
