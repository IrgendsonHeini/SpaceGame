...
TIE_FIGHTER         = load(path(space_wars_folder,"Space Wars assets", "Tie-Fighter.png"))
TIE_HUNTER          = load(path(space_wars_folder,"Space Wars assets", "Tie-Hunter.png"))
STAR_DESTROYER      = load(path(space_wars_folder,"Space Wars assets", "Star-Destroyer.png"))
DEATHSTAR           = load(path(space_wars_folder,"Space Wars assets", "Death-Star.png"))
MILLENNIUM_FALCON   = load(path(space_wars_folder,"Space Wars assets", "Millennium-Falcon.png"))
MILLENNIUM_FALCON_L = load(path(space_wars_folder,"Space Wars assets", "Millennium-Falcon-left.png"))
MILLENNIUM_FALCON_R = load(path(space_wars_folder,"Space Wars assets", "Millennium-Falcon-right.png"))
IMPERIAL_SHUTTLE    = load(path(space_wars_folder,"Space Wars assets", "Imperial-Shuttle.png"))
SLAVE_I             = load(path(space_wars_folder,"Space Wars assets", "Slave-I.png"))
...
class DeathrayExplosion(Ship):
    ...
    def draw(self,x,y):
        self.ship_img = EXPLOSION[int(self.counter)]
        screen.blit(self.ship_img,(x,y))       
        self.counter += 0.5
        if self.counter > 15:
            self.hp = 0
...
class Deathstar(Ship):
    ...
    def draw(self, window):
        ...
        if int(self.counter) == 11:
            self.target_location = (player.x+50,player.y+50)
            self.target_x        = player.x
            self.target_y        = player.y

        if self.counter > 0 and self.counter <= 14.5:
            window.blit(DEATHRAY[int(self.counter)],(self.x,self.y))
        
        if self.counter >= 15:
            window.blit(DEATHRAY[14],(self.x,self.y))
            pygame.draw.line(window,GREEN,(self.x+98,self.y+58),(self.target_x+50, self.target_y+50),4)
...
class Explosion:
    ...
    def draw(self, window):              
        ...
        window.blit(scale(self.img,(self.size,self.size)),(self.x,self.y))      
...
def main_menu():
    ...
    instruction2     = instruction_font.render('Press "   " to shoot und "   " to pause',1,WHITE)
    ...
    while active:
        ...
        if keys[pygame.K_s] or joystick.get_button(1):
            active = False
            game_loop()
...
# Steuerung Tastatur
        if pygame.joystick.get_count() == 0:
            ...
            if keys[pygame.K_LEFT]: #links
                player.x -= player_vel
                player.ship_img = MILLENNIUM_FALCON_L
                    
            if keys[pygame.K_RIGHT]: #rechts
                player.x +=  player_vel
                player.ship_img = MILLENNIUM_FALCON_R
                
            if not keys[pygame.K_RIGHT] and not keys[pygame.K_LEFT]: #animation aus
                player.ship_img = MILLENNIUM_FALCON
                
            if keys[pygame.K_LEFT] and keys[pygame.K_RIGHT]: #animation aus
                player.ship_img = MILLENNIUM_FALCON
                           
            if keys[pygame.K_UP]: #hoch
                player.y -= player_vel
                
            if keys[pygame.K_DOWN]: #runter
                player.y +=  player_vel
                
            if not keys[pygame.K_s]: #schuss freigeben
                released = True
            
            if keys[pygame.K_s]: #schießen
                if released:
                    player.shoot()
                    released = False            
...
def score_window():
    ...
    while active:
        ...
        if  keys[pygame.K_ESCAPE] or joystick.get_button(2):
            active = False
            pygame.quit()
