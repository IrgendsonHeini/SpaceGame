...
music.load("Space Wars assets/background-music.mp3")
music.set_volume(.7)
music.play(-1,0.0)
...
def game_loop():
    ...
    lost_font = pygame.font.SysFont("franklingothicmedium", 150)        
...
# Game over       
        ...
        if lost:
            ...
            if lost_count == 21:
                player.ship_img = EXPLOSION_09
            if lost_count == 24:
                player.ship_img = EXPLOSION_10
            lost_count += 1                
            if lost_count > FPS*3:
                active = False
                score_window()
                ...
                if level >= 6:
                    for i in range(int(wave_length/6)):
                        ...
                        for enemy in fleet:
                            if enemy.x == x and enemy.y == y:
                                fleet.remove(enemy)
                            if enemy.x+75 == x and enemy.y == y:
                                fleet.remove(enemy)
                            if enemy.x == x and enemy.y+75 == y:
                                fleet.remove(enemy)
                            if enemy.x+75 == x and enemy.y+75 == y:
                                fleet.remove(enemy)
...
# Schießen        
        player.move_lasers(-laser_vel*3, fleet)
...
def score_window():
    ...
    def highscore():
        ...
        label = font.render(f"{highscore_list[counter]}",1,ORANGE)
        WIN.blit(label,(WIDTH/2-label.get_width()/2,400 + counter*55))
        for i in highscore_list:
            WIN.blit(label,(WIDTH/2-label.get_width()/2,420 + counter*60))
            counter += 1
